...
// Ordered stage list for JS (template order, then orphan columns) so indices match DOM columns.
$stage_order_for_js = array();
$stage_keys_seen    = array();
foreach ( $stages as $s ) {
	$stage_order_for_js[] = array( 'key' => $s['key'], 'label' => $s['label'] );
	$stage_keys_seen[ $s['key'] ] = true;
}
foreach ( $by_stage as $key => $data ) {
	if ( ! isset( $stage_keys_seen[ $key ] ) ) {
		$stage_order_for_js[] = array( 'key' => $key, 'label' => $data['label'] );
		$stage_keys_seen[ $key ] = true;
	}
}
?>
<script>
